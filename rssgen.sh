...
# Prompt for episode info
EP_TITLE="${EP_TITLE_OPT:-${EP_TITLE:-}}"
EP_DESC="${EP_DESC_OPT:-${EP_DESC:-}}"
EP_URL="${EP_URL_OPT:-${EP_URL:-}}"
EP_DATE="${EP_DATE_OPT:-${EP_DATE:-}}"
EP_ITUNES_AUTHOR="${EP_ITUNES_AUTHOR_OPT:-${EP_ITUNES_AUTHOR_ENV}}"
EP_ITUNES_SUMMARY="${EP_ITUNES_SUMMARY_OPT:-${EP_ITUNES_SUMMARY_ENV}}"
EP_ITUNES_EXPLICIT="${EP_ITUNES_EXPLICIT_OPT:-${EP_ITUNES_EXPLICIT_ENV}}"
EP_ITUNES_DURATION="${EP_ITUNES_DURATION_OPT:-${EP_ITUNES_DURATION_ENV}}"
EP_ITUNES_IMAGE="${EP_ITUNES_IMAGE_OPT:-${EP_ITUNES_IMAGE_ENV}}"
EP_ITUNES_EPISODE="${EP_ITUNES_EPISODE_OPT:-${EP_ITUNES_EPISODE_ENV}}"
EP_ITUNES_SEASON="${EP_ITUNES_SEASON_OPT:-${EP_ITUNES_SEASON_ENV}}"
EP_ITUNES_EPISODETYPE="${EP_ITUNES_EPISODETYPE_OPT:-${EP_ITUNES_EPISODETYPE_ENV}}"
...
[ -z "$EP_DATE" ] && [ -t 0 ] && read -p "Episode Publication Date (YYYY-MM-DD, blank for today): " EP_DATE
if [ -z "$EP_DATE" ]; then
  EP_DATE=$(date -R)
else
  # macOS/BSD date compatibility
  if date -j -f "%Y-%m-%d" "$EP_DATE" "+%a, %d %b %Y %H:%M:%S %z" >/dev/null 2>&1; then
    EP_DATE=$(date -j -f "%Y-%m-%d" "$EP_DATE" "+%a, %d %b %Y %H:%M:%S %z")
  else
    EP_DATE=$(date -R)
  fi
fi
if [ "$PROMPT_ALL" -eq 1 ]; then
  ...
  [ -z "$EP_ITUNES_DURATION" ] && [ -t 0 ] && read -p "Episode iTunes Duration (e.g. 00:42:00): " EP_ITUNES_DURATION
...
# After gathering all input, check required fields
REQUIRED_MISSING=0
...
if [ -z "$EP_TITLE" ]; then
  echo "Error: Episode title is required." >&2
  REQUIRED_MISSING=1
fi
if [ -z "$EP_DESC" ]; then
  echo "Error: Episode description is required." >&2
  REQUIRED_MISSING=1
fi
if [ -z "$EP_URL" ]; then
  echo "Error: Episode audio URL is required." >&2
  REQUIRED_MISSING=1
fi
